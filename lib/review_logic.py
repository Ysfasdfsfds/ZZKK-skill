...
def product_quality_rows(module_name: str, *, final: bool, requirements: list[RequirementEntry] | None = None) -> list[tuple[str, str, str]]:
    haystack = '\n'.join(
        '\n'.join([
            requirement.title,
    ...
    if _contains(haystack, ('ki18n', '多语言', '国际化', '本地化', '地域', '时区')):
        if not final:
            return [
                ('性能效率', '多语言资源加载、文本处理、地域与时区数据查询应保持稳定响应，不得明显增加桌面基础组件启动和调用时延。', '重点关注多语言资源加载、地域格式转换、时区数据读取和高频文本处理场景。'),
                ('安全性', 'ki18n 组件重构应强化基础组件来源可控、构建可追溯和运行边界约束，不引入额外供应链安全风险。', '避免引入不可控外部依赖；组件替换、配置加载和资源访问需符合系统安全基线。'),
                ('可靠性', '重构后的多语言组件应在国际化适配、本地化文本处理、地域与时区数据管理等场景下保持结果一致。', '异常输入、缺失翻译资源、地域数据异常或时区数据不可用时应有稳定降级策略。'),
                ('兼容性（接口）', '重构组件应兼容现有桌面环境和系统组件对多语言能力的调用方式。', '接口名称、参数、返回值、配置路径和资源格式变化需有兼容或迁移说明。'),
                ('可维护性', '多语言资源、地域数据、时区数据和组件实现应具备清晰组织方式，便于后续版本维护和问题定位。', '资源文件、构建脚本、测试用例和替换策略应可追踪。'),
                ('可移植性', '组件应适配目标桌面操作系统版本和不同部署环境。', '不同语言、地区、时区配置下不得依赖人工修正才能完成基础功能。'),
                ('易用性', '系统语言、区域格式、时间显示和本地化文本应符合用户所在语言环境的使用习惯。', '用户可感知文本、格式和提示应保持一致、准确、可理解。'),
            ]
        return [
            ('性能效率', 'ki18n 重构后，多语言资源加载、本地化文本处理、地域格式转换和时区数据查询应保持稳定响应。', '需验证组件替代原生组件后通用功能无缺失、无偏差，且不引起明显启动、加载或查询性能退化。'),
            ('安全性', '组件来源、构建产物、运行依赖和资源加载路径应可控可追溯，降低基础组件供应链安全风险。', '重点验证外部依赖收敛、资源访问边界、异常输入处理和权限边界，不得扩大系统攻击面。'),
            ('可靠性', '国际化适配、本地化文本处理、地域与时区数据管理在异常资源、缺失翻译、边界日期和多语言切换场景下保持稳定。', '异常情况下应保留基础能力或给出可定位错误，不得导致调用方崩溃或核心桌面能力不可用。'),
            ('兼容性（接口）', '重构后的 ki18n 组件应兼容既有桌面环境、系统组件和应用对多语言基础能力的调用约定。', '接口、配置、资源格式和返回结果应与替代前保持一致；必要差异需有迁移说明和回归验证。'),
            ('可维护性', '多语言资源、地域数据、时区数据、构建脚本和测试用例应按组件边界清晰组织。', '便于后续补充语言资源、更新地域/时区数据、定位文本处理问题和持续演进组件。'),
            ('可移植性', '组件应适配银河麒麟桌面操作系统目标版本及其不同语言、地区和时区部署环境。', '环境差异不应影响通用多语言能力替代效果，跨语言/跨地区配置需可复现验证。'),
            ('易用性', '用户可见文本、日期时间、区域格式和语言切换结果应符合对应语言环境习惯。', '关键文案、格式展示和异常提示需保持一致、准确，避免用户因本地化差异产生误解。'),
        ]
    if not final:
        return [
