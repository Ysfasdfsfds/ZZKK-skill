...
- 初稿版本号
- 终稿版本号
- 客户需求说明书初稿撰写时间（不限制格式，按输入原样写入）
- 客户需求说明书评审表时间（不限制格式，按输入原样写入）
- 客户需求说明书终稿撰写时间（不限制格式，按输入原样写入）
...
- 客户需求说明书和产品需求说明书相关日期不做格式转换，按用户输入原样写入；两类说明书初稿首页和版本说明表只写初稿记录，终稿首页写初稿/评审/终稿三项日期，版本说明表写初稿和终稿两行
- 产品评审表项目名称和工作产品写 `银河麒麟桌面操作系统<产品名称>`，工作产品评审规模写对应初稿文件名，项目阶段写 `设计阶段`
...
python3 ~/.claude/skills/ZZKK-PRD/skill.py plan \
  ...
  --draft-version "<初稿版本号>" \
  --final-version "<终稿版本号>" \
  --customer-draft-date "<客户需求说明书初稿撰写时间>" \
  --customer-review-date "<客户需求说明书评审表时间>" \
  --customer-final-date "<客户需求说明书终稿撰写时间>" \
...
- 客户需求说明书和产品需求说明书初稿/评审/终稿日期写入对应首页、版本说明表和评审表位置
- 终稿内容包含对应评审记录编号，体现由初稿经评审形成终稿
